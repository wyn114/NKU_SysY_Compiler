...
factorial:
sub sp, sp, #4
str lr, [sp]    @Push lr（返回链接寄存器）到堆栈
sub sp, sp, #4
str r0, [sp]    @Push参数r0到堆栈，这个是函数的参数

cmp r0,#0        @对比r0 and 0的值
...
sub sp, sp, #4
str lr, [sp]     @保存lr到堆栈中
sub sp,sp,#4     @留出一个4字节空间，给用户输人保存

ldr r0,address_of_input_num  @传参，提示输人nm
bl printf                    @调用printf
